...
if (isNil "_container") exitWith {
    ["Loot crate", localize "$STR_antistasi_no_vehicles_near_by"] call A3A_fnc_customHint;
...
if (isNil "_override") then {
    if ((_leftover isEqualTo [[],[],[],[]]) or _allUnlocked) then {
        ["Loot crate", format [localize "$STR_antistasi_loot_transfered_to", getText (configFile >> "CfgVehicles" >> typeOf _container >> "displayname")]] call A3A_fnc_customHint;
    } else {
        ["Loot crate", format [localize "$STR_antistasi_loot_transfer_full", getText (configFile >> "CfgVehicles" >> typeOf _container >> "displayname")]] call A3A_fnc_customHint;
    };
    [_target, clientOwner, true] remoteExecCall ["A3A_fnc_canTransfer", 2];
};
