...
if (isNull _newBoss) exitWith {
	[_silent] spawn {
		params ["_silent"];
		sleep 5;
		private _textX = format [localize "STR_antistasi_new_commander_resigned"];
		if (!_silent) then {[petros,"hint",_textX, localize "STR_antistasi_new_commander"] remoteExec ["A3A_fnc_commsMP", 0]};
...
[_silent] spawn {
	params ["_silent"];
	sleep 5;
	private _textX = format [localize "STR_antistasi_new_commander_greet", name theBoss];
	if (!_silent) then {[petros,"hint",_textX, localize	"STR_antistasi_new_commander"] remoteExec ["A3A_fnc_commsMP", 0]};
	[] remoteExec ["A3A_fnc_statistics",[teamPlayer,civilian]];
};
