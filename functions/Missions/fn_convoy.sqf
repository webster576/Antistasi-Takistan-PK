...
switch (_convoyType) do
{
	case "Ammunition":
	{
		_textX = format [localize "$STR_antistasi_ammo_convoy_desc",_nameOrigin,_displayTime,_nameDest];
		_taskTitle = localize "$STR_antistasi_ammo_convoy_title";
		_taskIcon = "rearm";
		_typeVehObj = if (_sideX == Occupants) then {vehNATOAmmoTruck} else {vehCSATAmmoTruck};
	};
	case "Armor":
	{
		_textX = format [localize "$STR_antistasi_armored_convoy_desc",_nameOrigin,_displayTime,_nameDest];
		_taskTitle = localize "$STR_antistasi_armored_convoy_title";
		_taskIcon = "Destroy";
		_typeVehObj = if (_sideX == Occupants) then {vehNATOAA} else {vehCSATAA};
	};
	case "Prisoners":
	{
		_textX = format [localize "$STR_antistasi_prisoners_convoy_desc",_nameOrigin,_displayTime,_nameDest];
		_taskTitle = localize "$STR_antistasi_prisoners_convoy_title";
		_taskIcon = "run";
		_typeVehObj = if (_sideX == Occupants) then {selectRandom vehNATOTrucks} else {selectRandom vehCSATTrucks};
	};
	case "Reinforcements":
	{
		_textX = format [localize "$STR_antistasi_reinforcements_convoy_desc",_nameOrigin,_displayTime,_nameDest];
		_taskTitle = localize "$STR_antistasi_reinforcements_convoy_title";
		_taskIcon = "run";
		_typeVehObj = if (_sideX == Occupants) then {selectRandom vehNATOTrucks} else {selectRandom vehCSATTrucks};
	};
	case "Money":
	{
		_textX = format [localize "$STR_antistasi_money_convoy_desc",_nameOrigin,_displayTime,_nameDest];
		_taskTitle = localize "$STR_antistasi_money_convoy_title";
		_taskIcon = "move";
		_typeVehObj = "C_Van_01_box_F";
	};
	case "Supplies":
	{
		_textX = format [localize "$STR_antistasi_supply_convoy_desc",_nameOrigin,_displayTime,_nameDest,nameTeamPlayer];
		_taskTitle = localize "$STR_antistasi_supply_convoy_title";
		_taskIcon = "heal";
		_typeVehObj = "C_Van_01_box_F";
	};
};
